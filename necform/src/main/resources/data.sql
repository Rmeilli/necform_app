...
(gen_random_uuid(), 'Notion : organiser sa gestion de projet collaborative', 'Intelligence artificielle & Automatisations', 'Organisez vos projets et équipes avec Notion et ses fonctionnalités avancées.', 16, 2200) ON CONFLICT (titre) DO NOTHING;
...
(gen_random_uuid(), 'Administration Secure Transport', 'Cybersécurité', 'Gérez les transports sécurisés et les protocoles de chiffrement avancés.', 32, 4800) ON CONFLICT (titre) DO NOTHING;
...
(gen_random_uuid(), 'Tomcat', 'DevOps & Automatisation', 'Administration et configuration du serveur d''applications Tomcat.', 24, 3500) ON CONFLICT (titre) DO NOTHING;
...
(gen_random_uuid(), 'Google Cloud', 'Cloud Computing', 'Développez et déployez des applications sur Google Cloud Platform.', 40, 6000) ON CONFLICT (titre) DO NOTHING;
...
(gen_random_uuid(), 'SQL Server : initiation', 'Data, reporting et BI', 'Découvrez SQL Server et ses fonctionnalités de base de données.', 24, 3500) ON CONFLICT (titre) DO NOTHING;
...
(gen_random_uuid(), 'Lancer son activité en freelance', 'Métiers du numérique & développement', 'Apprenez à lancer et gérer votre activité de freelance.', 16, 2500) ON CONFLICT (titre) DO NOTHING;
...
(gen_random_uuid(), 'SEO : optimiser le référencement de votre site', 'Création de sites web', 'Optimisez votre site pour les moteurs de recherche et améliorez votre visibilité.', 16, 2800) ON CONFLICT (titre) DO NOTHING;
...
(gen_random_uuid(), 'Outlook', 'Bureautique & outils numériques', 'Gérez votre messagerie et calendrier avec Outlook.', 8, 1200) ON CONFLICT (titre) DO NOTHING;
...
(gen_random_uuid(), 'Silae', 'Logiciels métiers & logistique', 'Maîtrisez le logiciel Silae pour la gestion de la paie et des RH.', 32, 4500) ON CONFLICT (titre) DO NOTHING;
